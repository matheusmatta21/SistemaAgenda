...
def validarDados():
    ...
    if nome == "" or nome == " ":
        ...
        messagebox.showerror(
            title="Erro", message="Insira a formatação certa do e-mail!"
        )
        booleano = False
    elif preferencial == "" or preferencial == " ":
        messagebox.showerror(
            title="Erro", message="Selecione se você faz parte do grupo preferencial!"
        )
        booleano = False
    if booleano == True:
        escreverDados()
...
def lerDados():
    ...
    for linha in arquivo.readlines():
        ...
        labelContatos = Label(
            windowTwo,
            text=f"contato:{contato[0]},{contato[1]},{contato[2]},{contato[3]}",
        )
        labelContatos.pack()
